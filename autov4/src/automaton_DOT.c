...
void makeCharacterListWithSameAction(automaton_t *paut, char *printed,int first, state_t cur_state, state_t shift_state) {
	unsigned int i, same;
	...
	for (i=first+1, prec_same_i=first, nbsame=0 ; i<automaton_nb_characters(paut);i++) {
		...
		if (same) {
			printed[cur_state*automaton_nb_characters(paut)+i]=1;
			prec_same_i=i;
		} else {
			nbsame=0;
		}
